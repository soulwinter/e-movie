  ...
  <el-form class="filter-form">
    ...
    <tbody>
      <tr v-for="(item) in movies" :key="item.id">
        ...
        <td>{{ item.originalTitle ? (item.originalTitle.length > 30 ? item.originalTitle.substring(0, 30) + '...' : item.originalTitle) : '-' }}</td>
        <td>{{ item.originalLanguage || '-' }}</td>
        <td>{{ item.overview ? (item.overview.length > 30 ? item.overview.substring(0, 30) + '...' : item.overview) : '-' }}</td>
